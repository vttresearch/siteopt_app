 ...
 * - transforming data
 * - returning semantic ECharts options
 *
 * Visual styling such as:
 * - font sizes
 * - label rotation
 * - truncation
 * - grid spacing
 ...
 * @returns {Object|null} - Detection result with timeColumn and dataColumns
 */
...
function getLogOrLinearAxis(scaleType) {
  if (scaleType === "log") {
    return {
      type: "log",
      logBase: 10,
...
function buildMinHeightTooltipFormatter(params) {
  ...
  params.forEach((p) => {
    ...
    const num =
      typeof val === "number" ? val : (p.data?.value ?? p.value)
 ...
 * @returns {Object|null} - ECharts configuration
 */
...
export function getAggregationOptions(data, timeColumn) {
  ...
  const times = data
    .map((row) => new Date(row[timeColumn]))
...
export function generateColorPalette(count) {
  ...
  for (let i = baseColors.length; i < count; i += 1) {
...
export function detectScenarioStructure(data) {
  ...
  const scenarioCol = columns.find((c) =>
    scenarioKeywords.some((k) => c.toLowerCase().includes(k))
  )
...
/**
 * Build category-totals chart config.
 * Visual styling is intentionally minimal here.
 */
export function processCategorySummedData(
  data,
  scenarioStructure,
  scenarios,
  chartType,
  ...
  const categories = scenarioStructure.summaries?.length
    ? scenarioStructure.summaries
    : scenarioStructure.items

  if (!categories?.length) return null

  const columns = Object.keys(data[0])
  const scenarioColumn =
    columns.find((c) => /^(scenario|alternative|case|run)$/i.test(c)) || columns[0]
  const itemCol = columns.find((c) => /(item|parameter|variable|metric|name)/i.test(c))
  const valueCol = columns.find((c) => /(value|values?|result|total)/i.test(c))
  const categoryCol = columns.find((c) => /(category|summary|group|type)/i.test(c))
  ...
  const tooltip = {
    ...
    ...(useMinBarHeight && {
      formatter: buildMinHeightTooltipFormatter
    })
  }

  const valueAxis = getLogOrLinearAxis(yAxisScale)
  const categoryAxis = {
    type: "category",
...
/**
 * Build scenario comparison chart.
 * Visual styling is intentionally minimal here.
 */
export function processScenarioComparisonData(
  data,
  scenarioStructure,
  items,
  scenarios,
  ...
  if (!data?.length || !scenarioStructure || !items?.length || !scenarios?.length) {
    return null
  }
    ...
    items.map((item) => {
      if (itemCol && valueCol) {
        const rows = data.filter(
          (r) =>
            normalizeStr(r[scenarioColumn]) === scenarioName &&
            normalizeStr(r[itemCol]) === item
        )
        return rows.reduce((sum, row) => sum + (parseFloat(row[valueCol]) || 0), 0)
      }

      const row = data.find(
        (r) => normalizeStr(r[scenarioColumn]) === scenarioName
      )
  ...
  if (hideZeroValues) {
    const nonZeroIndices = items
      .map((_, i) => i)
      .filter((i) => valuesMatrix.some((row) => (row[i] ?? 0) !== 0))
  ...
  if (chartType === "horizontalBar") {
    ...
    base.xAxis = getLogOrLinearAxis(yAxisScale)
  } else {
    base.xAxis = {
      type: "category",
      data: itemsToShow
    }
    base.yAxis = getLogOrLinearAxis(yAxisScale)
  }
